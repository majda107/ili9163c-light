...
void ILI9163C_TFT::draw_triangle_z(int16_t x0, int16_t y0, int16_t z0, int16_t x1, int16_t y1, int16_t z1, int16_t x2, int16_t y2, int16_t z2, uint16_t color, int8_t (*buff)[128])
{ 
  // sorting
  if (y0 > y1) {
    short tmp_x = x0;
  ...
  if (y0 == y2) { // Handle awkward all-on-same-line case as its own thing
    ...
    this->draw_line_z(a, b + 1, y0, z0, z2, color, buff);
    return;
  }
  ...
  for (y = y0; y <= last; y++) {
    ...
    this->draw_line_z(a, b + 1, y, z0, z2, color, buff);
...
void ILI9163C_TFT::draw_line_z(int16_t x0, int16_t x1, int16_t y, int16_t z_left, int16_t z_right, uint16_t color, int8_t (*buff)[128])
{
  if(x0 < 0) x0 = 0;
  ...
  for(; x0 < x1; x0++)
  {
    ...
    if(z > buff[x0][y] / 10.0f)
    {
      this->set_pixel(x0, y, color);
      buff[x0][y] = (int8_t)(z * 10);
    }
  }
}
